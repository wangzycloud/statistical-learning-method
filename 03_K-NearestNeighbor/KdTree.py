...
"""
    ...
    https://github.com/wenffe/python-KD-
...
class KDTree(object):
    '''kd树'''
    def __init__(self, aList, labelList, func):
    ...
    def knn_algo(self, item, k=1):
        ...
        node_list = []
        distance = self.func(item,node.item)  # 测试点与最近点之间的距离
        least_dis = distance
        ...
        if node.left_child != None:
            ...
            left_dis = self.func(item,left_child.item)
        ...
        while True:
            ...
            par_dis = self.func(item,parent.item)
...
if __name__ == '__main__':
    ...
    t1 = time.time()
    kd_tree = KDTree(x_train,y_train,ManhattanDistance)
    t2 = time.time()

    correct_num = 0
    for i in range(x_test.shape[0]):
        label, node_list = kd_tree.knn_algo(x_test[i],k=5)
        print('点%s最接近的前k个点为:%s'%(x_test[i], node_list))
        print('点%s的标签:%s，预测为:%s\n'%([5.3,4,2,1.2,5],y_test[i],label))
        if label == y_test[i]:
            correct_num += 1
